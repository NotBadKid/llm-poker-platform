...
from pokerkit.state import State, Card
    ...
    try:
        ...
        while True:
            ...
            if hasattr(state, 'deck'):
                deck_content = [str(c) for c in state.deck]
                print(f"[DEBUG] Full Deck ({len(deck_content)} cards): {deck_content}")

            custom_deck_strings = [
                '2h', '7d', 'Qs', 'Ac',  # Te będą na spodzie talii (rozdane na końcu)
                # ... tu reszta kart ...
                'As', 'Kd', 'Th', '9s'  # Te są na "górze" talii (rozdane jako pierwsze)
            ]

            # 2. Skonwertuj stringi na obiekty Card
            # Używamy Card(s), który parsuje stringi typu 'As', 'Td' itp.
            custom_deck_objects = [next(Card.parse(s)) for s in custom_deck_strings]

            # 3. Nadpisz talię w stanie gry
            state.deck = custom_deck_objects

            # Opcjonalnie: sprawdź czy zadziałało
            print(f"[DEBUG] Manual Deck Injected: {[str(c) for c in state.deck]}")
